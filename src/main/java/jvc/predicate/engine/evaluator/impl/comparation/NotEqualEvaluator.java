...
public class NotEqualEvaluator<L extends PLAtomic<?>, R extends PLAtomic<?>> extends BinaryEvaluator<L, R, PLBoolean> {

    public NotEqualEvaluator() {
    }

    public NotEqualEvaluator(Evaluator<L> left, Evaluator<R> right) throws Exception {
        super(left, right);
    }
            ...
            case PLType.FLOAT:
                ...
                switch (right.getType()) {
                    ...
                    case PLType.INT:
                        ...
                        result = new PLBoolean(!Objects.equals(leftInt, right.toInt().getData()));

                        break;

                    case PLType.FLOAT:

                        result = new PLBoolean(!Objects.equals(leftInt.floatValue(), right.toFloat().getData()));

                        break;

                    default: // string

                        result = new PLBoolean(true);
